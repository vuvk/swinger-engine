...
public final class Ray {
     ...
     */
    public double getSolid(Point mapPoint, Vector2 collisionPoint) {
        mapPoint.setLocation((int)start.x, (int)start.y);
        ...
        while (true) {
            ...
            if ((mapPoint.x < 0 || mapPoint.x >= Map.WIDTH ||
                ...
                (Map.SOLIDS[mapPoint.x][mapPoint.y])/* ||
                Map.SEGMENTS[mapPoint.x][mapPoint.y] != null*/) {
                break;
            }
             
            // луч столкнулся с сегментом
            Segment mapSegment = Map.SEGMENTS[mapPoint.x][mapPoint.y];            
            if (mapSegment != null) {
                ...
                if (point != null) {
                    ...
                    Vector2 plane = Player.getInstance().getPlane();
